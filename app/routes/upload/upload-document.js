...
const { uploadDocumentFile } = require('../../storage/document-blob-repository')
...
},
{
  ...
  options: {
    ...
    handler: async (request, h) => {
      ...
      await uploadDocumentFile(filename, stream)
      ...
      return h.redirect(`${uploadConstants.routes.document.redirect}?name=${filename}`)
    }
  }
}]
